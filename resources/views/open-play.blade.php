...
(() => {
    ...
    function stageTeamCourtWithHistory(match){
        ...
        const nonRepeatingCandidates=candidates.filter(candidate=>!candidate.immediateRepeat),selection=nonRepeatingCandidates.length?nonRepeatingCandidates:candidates;
    ...
    function showPlayerPicker(match,index,fromStaged=false){
        ...
        const selected=match.manualSlots[index],s=state.current,available=s.waiting.filter(player=>(player.status||'ready')==='ready'&&(!selected||s.mode!=='team'||player.teamIndex===selected.teamIndex)),currentPlayers=(match.manualSlots||[]).filter(player=>player&&player.id!==selected?.id),fresh=newOpponentsOnly(available,currentPlayers,s);
        const render=(choices,showingAll=false)=>{
            const showingFresh=!showingAll&&fresh.length>0,description=showingFresh?'Players with no previous match against the players already selected':'All eligible players';
            playerPickerSheet.innerHTML=`<div class="player-picker-head"><h3 id="player-picker-title">Pick player</h3><p>${description}</p></div>${selected?'<button class="clear-manual-slot" id="clear-manual-slot">Clear this slot</button>':''}<span class="picker-label">Available · ${choices.length}</span><div>${choices.map(p=>{const mins=Math.max(0,Math.floor((Date.now()-new Date(p.queuedAt||s.startedAt))/60000));return `<button class="picker-player" data-picker-player="${p.id}" type="button"><span class="waiting-avatar">${escapeHtml(playerInitials(p))}</span><span><strong>${escapeHtml(p.name)}</strong><small>${'★'.repeat(playerRating(p))} · ${p.gamesPlayed||0} games</small></span><span class="picker-wait">${mins}m</span></button>`}).join('')}</div>${showingFresh&&fresh.length<available.length?'<button class="clear-manual-slot" id="show-all-picker-players">Show all eligible players</button>':''}<div class="picker-cancel-wrap"><button class="winner-cancel" id="picker-cancel" type="button">Cancel</button></div>`;
            $$('[data-picker-player]',playerPickerSheet).forEach(b=>b.onclick=()=>selectManualPlayer(b.dataset.pickerPlayer));const clear=$('#clear-manual-slot');if(clear)clear.onclick=clearManualSlot;const showAll=$('#show-all-picker-players');if(showAll)showAll.onclick=()=>render(available,true);$('#picker-cancel').onclick=dismissPlayerPicker;
        };
        render(fresh.length?fresh:available,!fresh.length);playerPicker.hidden=false;
    }
