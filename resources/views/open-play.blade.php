...
<head>
    ...
    <style>
        ...
        * { box-sizing:border-box; }
        ...
        .skill[data-rating="0"] { grid-column:1 / -1; border-color:#cbd5e1; background:#f8fafc; color:#64748b; }
        ...
        .skill.selected[data-rating="0"] { background:linear-gradient(135deg,#94a3b8,#64748b); }
        ...
        .team-logo { width:32px; height:32px; flex:none; border:1px solid var(--line); border-radius:50%; background:var(--card); object-fit:cover; }
        .summary-player .team-logo { display:inline-block; width:18px; height:18px; margin-right:.35rem; vertical-align:middle; }
        .standing-name .team-logo { display:inline-block; width:24px; height:24px; margin-right:.45rem; vertical-align:middle; }
        .team-logo-upload { flex:none; color:var(--blue); cursor:pointer; font-size:.68rem; font-weight:750; white-space:nowrap; }
        .team-logo-upload input { display:none; }
        ...
        .court-slot-wait { color:#7b8596; font-size:.55rem; font-weight:700; }
        ...
        .up-next-team { position:relative; overflow:hidden; padding:.4rem .55rem; }
        .up-next-team > :not(.up-next-team-logo) { position:relative; z-index:1; }
        .up-next-team-logo { position:absolute; z-index:0; top:50%; right:.45rem; width:74px; height:74px; opacity:.16; object-fit:contain; pointer-events:none; transform:translateY(-50%); }
        ...
        .pair-warning { margin:.35rem 0 0; padding:.28rem .35rem; border-radius:6px; background:#fff0cf; color:#9a6200; font-size:.59rem; font-weight:800; line-height:1.35; }
        ...
        .waiting-list { max-height:470px; overflow-y:auto; scrollbar-gutter:stable; border:1px solid #e0e5ec; border-radius:11px; background:#fff; }
        ...
        .match-history-toolbar { display:flex; align-items:center; justify-content:space-between; gap:.7rem; margin-bottom:.45rem; }
        .match-history-toolbar .match-history-title { margin-bottom:0; }
        .match-history-search { width:min(250px,55%); padding:.48rem .7rem; font-size:.76rem; }
        .match-history-empty { padding:1rem; color:var(--muted); font-size:.8rem; text-align:center; }
        .match-history-card { max-height:590px; overflow-y:auto; scrollbar-gutter:stable; border:1px solid #dbe1e9; border-radius:11px; background:#fff; }
        ...
        .history-edit { justify-self:end; padding:.2rem .35rem; border:1px solid #cbd5e1; border-radius:5px; background:#fff; color:#526174; cursor:pointer; font-size:.58rem; font-weight:800; }
        ...
        .team-standings-large .summary-table-head,.team-standings-large .summary-player { grid-template-columns:4.2rem minmax(8rem,.7fr) minmax(0,1fr) 4.3rem 4.3rem 4.8rem 4.8rem; padding:.9rem 1rem; gap:.55rem; }
        ...
        .team-standings-large .team-rank-cell { display:flex; align-items:center; justify-content:center; }
        .team-standings-large .team-rank-cell .standing-rank { font-size:1.15rem; }
        .team-standings-large .team-rank-cell .standing-rank.leader { width:52px; height:52px; font-size:1.65rem; }
        .team-standings-large .team-rank-details { display:grid; min-width:0; text-align:left; }
        .team-standings-large .team-rank-details strong { overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
        .team-standings-large .team-rank-details small { color:var(--muted); font-weight:650; }
        .team-standings-large .team-cell { display:flex; justify-content:center; }
        .team-standings-large .team-cell .team-logo { width:72px; height:72px; margin:0; }
        ...
        .team-player-summary .summary-table-head,.team-player-summary .summary-player { grid-template-columns:1.8rem minmax(0,1fr) 2.7rem 1.7rem 1.7rem 3.6rem; }
        .team-player-summary .summary-player.top-1 { background:linear-gradient(90deg,#fff8d9,#fff 55%); box-shadow:inset 4px 0 #f2bc24; }
        .team-player-summary .summary-player.top-2 { background:linear-gradient(90deg,#f1f5f9,#fff 55%); box-shadow:inset 4px 0 #9caabd; }
        .team-player-summary .summary-player.top-3 { background:linear-gradient(90deg,#fff0e5,#fff 55%); box-shadow:inset 4px 0 #cf7c43; }
        ...
        html[data-theme="dark"] .team-player-summary .summary-player.top-1 { background:linear-gradient(90deg,#3c3212,#182231 58%); }
        html[data-theme="dark"] .team-player-summary .summary-player.top-2 { background:linear-gradient(90deg,#293342,#182231 58%); }
        html[data-theme="dark"] .team-player-summary .summary-player.top-3 { background:linear-gradient(90deg,#3a281d,#182231 58%); }
...
<script>
(() => {
    const initial = { screen:'home', setup:{ name:'', eventDate:'', eventEndDate:'', courts:2, mode:'balanced', teamCount:null, teamSize:4, teamNames:[], teamColors:[], teamLogos:[], gamesPerPlayer:4, points:11, pointsOn:true, timer:false }, players:[], rating:0, current:null, history:[], keepHistory:true };
    // `crypto.randomUUID()` is unavailable in some browsers or non-secure contexts.
    const newId = () => {
    ...
    state.setup.teamLogos = Array.isArray(state.setup.teamLogos) ? state.setup.teamLogos : [];
    ...
    state.rating = Math.round(Math.min(5, Math.max(0, Number(state.rating ?? state.skill ?? 0))));
    ...
    const archiveSession = session => fetch({{ Illuminate\Support\Js::from(route('open-play.sessions.archive')) }},{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json','X-CSRF-TOKEN':csrfToken},body:JSON.stringify({session})}).then(async response=>{if(!response.ok)throw new Error(`Unable to archive session (${response.status})`);return response.json();});
    ...
    const ratingLabel = rating => rating ? `${rating}-star` : 'Unranked';
    function renderSkills(){ $('#default-skill').textContent=ratingLabel(state.rating); $('#skills').innerHTML=ratings.map(r=>r===0?`<button class="skill ${r===state.rating?'selected':''}" data-rating="0" aria-label="Unranked, no star rating">Unranked<small>—</small></button>`:`<button class="skill ${r===state.rating?'selected':''}" data-rating="${r}" aria-label="${r} star rating">${r}<small>${'★'.repeat(r)}</small></button>`).join(''); $$('#skills .skill').forEach(b=>b.onclick=()=>{state.rating=+b.dataset.rating;renderSkills();save();}); }
    ...
    function playerAssignment(player,session=state.current){
        if(!player||!session)return null;
        for(const match of session.matches||[]){
            if([...(match.teamA||[]),...(match.teamB||[]),...(match.manualSlots||[])].some(item=>item?.id===player.id))return match.started?'Playing':'Staged';
            if((match.nextQueue||[]).some(item=>item?.id===player.id))return 'Queued';
        }
        if((session.upNext||[]).some(match=>[...(match.teamA||[]),...(match.teamB||[])].some(item=>item?.id===player.id)))return 'Queued';
        return null;
    }
    function autoQueueEligiblePlayers(session=state.current){
        if(!session)return [];
        const seen=new Set();
        return (session.waiting||[]).filter(player=>{
            const canonical=(session.players||[]).find(item=>item.id===player.id),gamesPlayed=canonical?.gamesPlayed??player.gamesPlayed??0;
            if(seen.has(player.id)||!isAvailable(player)||!isAvailable(canonical||player)||gamesPlayed>=session.gamesPerPlayer||playerAssignment(player,session))return false;
            seen.add(player.id);
            return true;
        });
    }
    function canMakeTeamMatch(session=state.current){
        if(!session)return false;
        const eligible=autoQueueEligiblePlayers(session),groups=Array.from({length:session.teamCount||0},(_,index)=>eligible.filter(player=>player.teamIndex===index));
    ...
    function teamLogo(index,source=state.setup){return source?.teamLogos?.[index]||null;}
    function teamLogoUrl(logo){return typeof logo==='string'?logo:logo?.url||'';}
    function teamLogoMarkup(index,source){const url=teamLogoUrl(teamLogo(index,source));return url?`<img class="team-logo" src="${escapeHtml(url)}" alt="">`:'';}
    function teamLogoUpload(index,source){const logo=teamLogoMarkup(index,source==='active'?state.current:state.setup);return `<label class="team-logo-upload${logo?' has-logo':''}">${logo||'Logo'}<input type="file" accept="image/*" data-team-logo="${index}" data-team-logo-source="${source}"></label>`;}
    ...
    const completedTeammatePairCount = (first,second,session=state.current) => {
        if(!first||!second)return 0;
        const names=[first.name,second.name].map(name=>String(name).toLocaleLowerCase()).sort().join('|');
        return (session?.completedMatches||[]).reduce((count,match)=>count+['teamA','teamB'].filter(side=>(match[side]||[]).map(name=>String(name).toLocaleLowerCase()).sort().join('|')===names).length,0);
    };
    ...
    function createTeamPairPlan(players,teamCount,gamesPerPlayer){
        return Array.from({length:teamCount||0},(_,teamIndex)=>{
            let circle=players.filter(player=>player.teamIndex===teamIndex).sort((a,b)=>String(a.id).localeCompare(String(b.id)));
            if(circle.length<2)return [];
            if(circle.length%2)circle=[...circle,null];
            const rounds=Math.min(Math.max(0,Number(gamesPerPlayer)||0),circle.length-1),pairs=[];
            for(let round=0;round<rounds;round++){
                for(let index=0;index<circle.length/2;index++){const first=circle[index],second=circle[circle.length-1-index];if(first&&second)pairs.push([first.id,second.id].sort().join('|'));}
                const fixed=circle[0],rotating=circle.slice(1);rotating.unshift(rotating.pop());circle=[fixed,...rotating];
            }
            return pairs;
        });
    }
    ...
    function stageTeamCourtWithHistory(match){
        ...
        if(!s||match.teamA.length||match.manualSlots)return;
        const eligible=autoQueueEligiblePlayers(s),groups=Array.from({length:s.teamCount||0},(_,index)=>({index,players:eligible.filter(player=>player.teamIndex===index)})).filter(group=>group.players.length>=2),recentPairs=new Set(s.lastTeammatePairKeys||[]),candidates=[];
        for(let left=0;left<groups.length;left++)for(let right=left+1;right<groups.length;right++)for(const teamA of twoPlayerChoices(groups[left].players))for(const teamB of twoPlayerChoices(groups[right].players)){
            const firstPairKey=playerPairKey(teamA[0],teamA[1]),secondPairKey=playerPairKey(teamB[0],teamB[1]),teammate=(s.teammateHistory?.[firstPairKey]||0)+(s.teammateHistory?.[secondPairKey]||0),opponents=teamA.reduce((sum,a)=>sum+teamB.reduce((inner,b)=>inner+(s.opponentHistory?.[playerPairKey(a,b)]||0),0),0),matchup=s.matchupCounts?.[matchupKey(groups[left].index,groups[right].index)]||0;
            const immediateRepeat=recentPairs.has(firstPairKey)||recentPairs.has(secondPairKey);
            candidates.push({teamA,teamB,a:groups[left].index,b:groups[right].index,pairKeys:[firstPairKey,secondPairKey],immediateRepeat,score:matchup*100+teammate*30+opponents*12+(immediateRepeat?60:0)+Math.random()*4});
        }
        if(!candidates.length){toast('Need two available players from two different teams');return;}
        // Fairness always comes first: select the least-played players, then the players
        // who have waited longest. Round-robin history only resolves an otherwise fair tie.
        const compareFairness=(first,second)=>{
            const orderPlayers=candidate=>[...candidate.teamA,...candidate.teamB].sort((a,b)=>(a.gamesPlayed||0)-(b.gamesPlayed||0)||queuedAtTime(a,s)-queuedAtTime(b,s)||String(a.id).localeCompare(String(b.id)));
            const firstPlayers=orderPlayers(first),secondPlayers=orderPlayers(second);
            for(let index=0;index<firstPlayers.length;index++){
                const gameDifference=(firstPlayers[index].gamesPlayed||0)-(secondPlayers[index].gamesPlayed||0);
                if(gameDifference)return gameDifference;
            }
            for(let index=0;index<firstPlayers.length;index++){
                const waitDifference=queuedAtTime(firstPlayers[index],s)-queuedAtTime(secondPlayers[index],s);
                if(waitDifference)return waitDifference;
            }
            return 0;
        };
        // Keep each team's pre-built round-robin plan intact whenever both planned pairs
        // are available. This prevents the final rounds from stranding a repeated pair.
        // Fairness still chooses among all valid planned matches.
        const freshPairingCandidates=candidates.filter(candidate=>canUseTeamPair(candidate.teamA[0],candidate.teamA[1],s)&&canUseTeamPair(candidate.teamB[0],candidate.teamB[1],s));
        const remainingPlannedPairs=new Map(groups.map(group=>[group.index,new Set((s.teamPairPlan?.[group.index]||[]).filter(key=>(s.teammateHistory?.[key]||0)===0))]));
        const plannedPairingCandidates=freshPairingCandidates.filter(candidate=>remainingPlannedPairs.get(candidate.a)?.has(candidate.pairKeys[0])&&remainingPlannedPairs.get(candidate.b)?.has(candidate.pairKeys[1]));
        const selection=plannedPairingCandidates.length?plannedPairingCandidates:(freshPairingCandidates.length?freshPairingCandidates:[...candidates]);
        selection.sort((a,b)=>compareFairness(a,b)||a.score-b.score);
        const chosen=selection[0],ids=new Set([...chosen.teamA,...chosen.teamB].map(player=>player.id));
        s.lastTeammatePairKeys=chosen.pairKeys;
        s.teammateHistory=s.teammateHistory||{};s.opponentHistory=s.opponentHistory||{};
        [[chosen.teamA[0],chosen.teamA[1]],[chosen.teamB[0],chosen.teamB[1]]].forEach(pair=>{const key=playerPairKey(...pair);s.teammateHistory[key]=(s.teammateHistory[key]||0)+1;});
        chosen.teamA.forEach(a=>chosen.teamB.forEach(b=>{const key=playerPairKey(a,b);s.opponentHistory[key]=(s.opponentHistory[key]||0)+1;}));
        const key=matchupKey(chosen.a,chosen.b);s.matchupCounts=s.matchupCounts||{};s.matchupCounts[key]=(s.matchupCounts[key]||0)+1;match.teamA=chosen.teamA;match.teamB=chosen.teamB;match.teamAIndex=chosen.a;match.teamBIndex=chosen.b;match.matchupKey=key;s.waiting=s.waiting.filter(player=>!ids.has(player.id));save();renderActive();
    }
    ...
    const queuedAtTime = (player,session=state.current) => { const time=new Date(player.queuedAt||session?.startedAt||0).getTime(); return Number.isFinite(time)?time:0; };
    const longestWaitingFirst = (players,session=state.current) => [...players].sort((a,b)=>queuedAtTime(a,session)-queuedAtTime(b,session)||a.name.localeCompare(b.name));
    const waitMinutes = (player,session=state.current) => Math.max(0,Math.floor((Date.now()-queuedAtTime(player,session))/60000));
    const balancedPairKey = (a,b) => [a.id,b.id].sort().join('|');
    function balancedTeams(players,s){const options=[[[0,1],[2,3]],[[0,2],[1,3]],[[0,3],[1,2]]];s.balancedPairHistory=s.balancedPairHistory||{};const ratingGap=(a,b)=>!playerRating(a)||!playerRating(b)?0:Math.abs(playerRating(a)-playerRating(b));const scored=options.map(([a,b])=>{const pairs=[[players[a[0]],players[a[1]]],[players[b[0]],players[b[1]]]],repeats=pairs.reduce((sum,pair)=>sum+(s.balancedPairHistory[balancedPairKey(...pair)]||0),0),gap=pairs.reduce((sum,pair)=>sum+ratingGap(...pair),0);return {pairs,score:repeats*100+gap*8+Math.random()*6};}).sort((a,b)=>a.score-b.score)[0];scored.pairs.forEach(pair=>{const key=balancedPairKey(...pair);s.balancedPairHistory[key]=(s.balancedPairHistory[key]||0)+1;});return {teamA:scored.pairs[0],teamB:scored.pairs[1]};}
    function stageCourt(match){ const s=state.current,available=autoQueueEligiblePlayers(s),queued=(match.nextQueue||[]).filter(p=>(p.status||'ready')==='ready'),source=queued.length>=4?queued:available;if(match.teamA.length||match.manualSlots||source.length<4)return;const four=(s.mode==='balanced'?longestWaitingFirst(source,s):shuffled(source)).slice(0,4),teams=s.mode==='balanced'?balancedTeams(four,s):{teamA:[four[0],four[3]],teamB:[four[1],four[2]]};match.teamA=teams.teamA;match.teamB=teams.teamB;const ids=new Set(four.map(p=>p.id));s.waiting=s.waiting.filter(p=>!ids.has(p.id));match.nextQueue=(match.nextQueue||[]).filter(p=>!ids.has(p.id));match.started=false;match.startedAt=null;match.completedAt=null;match.done=false;save();renderActive(); }
    ...
    const playerAvatarColor = player => ['#458fe0','#9b51c6','#d04438','#20b76d'][Math.max(0,state.current?.players.findIndex(item=>item.id===player.id)??0)%4];
    function courtAvatar(player){const color=playerTeamColor(player)||(state.current?.mode==='balanced'?playerAvatarColor(player):null);return `<span class="court-avatar"${color?` style="--player-team-color:${color}"`:''}>${escapeHtml(playerInitials(player))}</span>`;}
    function courtPlayer(player){ const balanced=state.current?.mode==='balanced';return `<div class="court-slot">${state.current?.mode==='team'?'':`<span class="court-slot-rating">${playerRating(player)}.0</span>`}${courtAvatar(player)}<span class="court-player-name">${escapeHtml(player.name)}</span>${balanced?`<small class="court-slot-wait">${waitMinutes(player)}m wait</small>`:''}</div>`; }
    function editableCourtPlayer(player,index,matchId){const balanced=state.current?.mode==='balanced';return `<button class="manual-slot court-slot" data-edit-staged-slot="${index}" data-edit-staged-match="${matchId}" type="button" aria-label="Change ${escapeHtml(player.name)}">${state.current?.mode==='team'?'':`<span class="court-slot-rating">${playerRating(player)} ★</span>`}${courtAvatar(player)}<span class="court-player-name">${escapeHtml(player.name)}</span>${balanced?`<small class="court-slot-wait">${waitMinutes(player)}m wait</small>`:''}</button>`;}
    ...
    function addUpNext(){const s=state.current,available=autoQueueEligiblePlayers(s);if(s.mode==='team'){const next={id:newId(),teamA:[],teamB:[]};stageTeamCourtWithHistory(next);if(next.teamA.length===2&&next.teamB.length===2){s.upNext.push(next);save();renderActive();}return;}if(available.length<4)return;const picked=(s.mode==='balanced'?longestWaitingFirst(available,s):shuffled(available)).slice(0,4),ids=new Set(picked.map(p=>p.id)),teams=s.mode==='balanced'?balancedTeams(picked,s):{teamA:[picked[0],picked[3]],teamB:[picked[1],picked[2]]};s.waiting=s.waiting.filter(p=>!ids.has(p.id));s.upNext.push({id:newId(),...teams});save();renderActive();}
    ...
    const playerGameCount = (player,session=state.current) => session?.players.find(item=>item.id===player.id)?.gamesPlayed??player.gamesPlayed??0;
    const leastPlayedThenLongestWaiting = (players,session=state.current) => [...players].sort((a,b)=>playerGameCount(a,session)-playerGameCount(b,session)||queuedAtTime(a,session)-queuedAtTime(b,session)||a.name.localeCompare(b.name));
    const unusedTeammateChoices = (players,teammate,session=state.current) => {
        if(!teammate)return players;
        const unused=players.filter(player=>teammatePairCount(player,teammate,session)===0);
        return unused.length?unused:players;
    };
    ...
    function autoFillUpNextPlayer(nextId,side){
        ...
        const eligible=autoQueueEligiblePlayers(s).filter(player=>player.teamIndex===teamIndex);
        const choices=unusedTeammateChoices(eligible,teammate,s);
        if(!choices.length){toast(`No available player remains for ${s.teamNames?.[teamIndex]||teamPalette[teamIndex]?.[0]||`Team ${teamIndex+1}`}.`);return;}
        const player=leastPlayedThenLongestWaiting(choices,s)[0];
        s.waiting=s.waiting.filter(item=>item.id!==player.id);
        team.push(player);
        save();
        renderActive();
    ...
    function autoFillManual(match){
        ...
        if(s.mode!=='team'){
            const available=autoQueueEligiblePlayers(s);
        ...
        for(const slot of emptySlots){
            ...
            const eligible=autoQueueEligiblePlayers(s).filter(player=>player.teamIndex===teamIndex);
            const choices=unusedTeammateChoices(eligible,teammate,s);
            if(!choices.length){match.manualSlots=originalSlots;s.waiting=originalWaiting;toast(`No available player remains for ${s.teamNames?.[teamIndex]||teamPalette[teamIndex]?.[0]||`Team ${teamIndex+1}`}.`);return;}
            const player=leastPlayedThenLongestWaiting(choices,s)[0];
            s.waiting=s.waiting.filter(item=>item.id!==player.id);
    ...
    function showPlayerPicker(match,index,fromStaged=false){
        pickerMatchId=match.id;pickerSlotIndex=index;pickerWasStaged=fromStaged;
        const selected=match.manualSlots[index],s=state.current,waiting=s.waiting.filter(player=>(player.status||'ready')==='ready'&&hasGameRemaining(player,s)),currentPlayers=(match.manualSlots||[]).filter(player=>player&&player.id!==selected?.id),teamRestriction=selected&&s.mode==='team',choices=teamRestriction?waiting.filter(player=>player.teamIndex===selected.teamIndex):waiting,sameSide=index<2?[0,1]:[2,3],prospectiveTeammate=match.manualSlots[sameSide.find(slot=>slot!==index)];
        const historyLabel=player=>{
            const labels=[];
            if(prospectiveTeammate){const count=s.teammateHistory?.[playerPairKey(player,prospectiveTeammate)]||0;if(count)labels.push(`Paired with ${prospectiveTeammate.name} · ${count}×`);}
            const opponentCount=currentPlayers.filter(current=>current!==prospectiveTeammate).reduce((total,current)=>total+(s.opponentHistory?.[playerPairKey(player,current)]||0),0);
            if(opponentCount)labels.push(`Played against selected players · ${opponentCount}×`);
            return labels.length?labels.join(' · '):'No previous pairing with this court';
        };
        const description=teamRestriction?`Showing all ${choices.length} waiting players from ${s.teamNames?.[selected.teamIndex]||teamPalette[selected.teamIndex]?.[0]||'this team'}`:`Showing all ${choices.length} waiting players`;
        const emptyMessage=teamRestriction?'No waiting player from this team. Players shown as Staged, Queued, or Playing are already assigned.':'No waiting players are available. Players shown as Staged, Queued, or Playing are already assigned.';
        playerPickerSheet.innerHTML=`<div class="player-picker-head"><h3 id="player-picker-title">Pick player</h3><p>${description}</p></div>${selected?'<button class="clear-manual-slot" id="clear-manual-slot">Clear this slot</button>':''}<span class="picker-label">Waiting · ${choices.length}</span><div>${choices.length?choices.map(player=>{const mins=Math.max(0,Math.floor((Date.now()-new Date(player.queuedAt||s.startedAt))/60000));return `<button class="picker-player" data-picker-player="${player.id}" type="button"><span class="waiting-avatar">${escapeHtml(playerInitials(player))}</span><span><strong>${escapeHtml(player.name)}</strong><small>${historyLabel(player)}</small></span><span class="picker-wait">${mins}m</span></button>`}).join(''):`<p class="hint">${emptyMessage}</p>`}</div><div class="picker-cancel-wrap"><button class="winner-cancel" id="picker-cancel" type="button">Cancel</button></div>`;
    ...
    function showPlayerPicker(match,index,fromStaged=false){
        ...
        const availablePlayers=autoQueueEligiblePlayers(s),available=player=>availablePlayers.some(item=>item.id===player.id);
        ...
        const hasUnusedTeammate=Boolean(prospectiveTeammate&&availablePlayers.some(player=>player.teamIndex===prospectiveTeammate.teamIndex&&teammatePairCount(player,prospectiveTeammate,s)===0));
        const canSelect=player=>available(player)&&(!hasUnusedTeammate||!prospectiveTeammate||teammatePairCount(player,prospectiveTeammate,s)===0);
        const orderedPlayers=[...players].sort((a,b)=>Number(canSelect(b))-Number(canSelect(a))||playerGameCount(a,s)-playerGameCount(b,s)||queuedAtTime(a,s)-queuedAtTime(b,s)||a.name.localeCompare(b.name));
        ...
        playerPickerSheet.innerHTML=`<div class="player-picker-head"><h3 id="player-picker-title">Pick player</h3><p>${description}</p></div>${selected?'<button class="clear-manual-slot" id="clear-manual-slot">Clear this slot</button>':''}<span class="picker-label">Team players · ${players.length}</span><div>${players.length?orderedPlayers.map(player=>{const isAvailable=available(player),selectable=canSelect(player),mins=Math.max(0,Math.floor((Date.now()-new Date(player.queuedAt||s.startedAt))/60000)),status=!isAvailable?assignment(player):selectable?'Available':'Use an unused teammate pairing first';return `<button class="picker-player" data-picker-player="${player.id}" type="button" ${selectable?'':'disabled'}><span class="waiting-avatar">${escapeHtml(playerInitials(player))}</span><span><strong>${escapeHtml(player.name)}</strong><small>${selectable?`Available · ${playerGameCount(player,s)} games · waiting ${mins}m`:escapeHtml(status)}</small></span><span class="picker-wait">${selectable?'Select':'Unavailable'}</span></button>`;}).join(''):'<p class="hint">No players are eligible for this match.</p>'}</div><div class="picker-cancel-wrap"><button class="winner-cancel" id="picker-cancel" type="button">Cancel</button></div>`;
    ...
    function selectManualPlayer(playerId){
        ...
        const sameSide=pickerSlotIndex<2?[0,1]:[2,3],teammate=match.manualSlots[sameSide.find(slot=>slot!==pickerSlotIndex)],eligible=autoQueueEligiblePlayers(s).filter(candidate=>!teammate||s.mode!=='team'||candidate.teamIndex===teammate.teamIndex),choices=unusedTeammateChoices(eligible,teammate,s);
        if(!choices.some(candidate=>candidate.id===player.id)){toast('Choose an unused teammate pairing while one is available.');return;}
    ...
    function playerCopies(id){const s=state.current;if(!s)return[];const copies=[state.players.find(p=>p.id===id),s.players.find(p=>p.id===id),s.waiting.find(p=>p.id===id),...(s.upNext||[]).flatMap(next=>[...(next.teamA||[]),...(next.teamB||[])]),...s.matches.flatMap(m=>[...(m.teamA||[]),...(m.teamB||[]),...(m.manualSlots||[]),...(m.nextQueue||[])]).filter(p=>p?.id===id)];return [...new Set(copies.filter(Boolean))];}
    function updatePlayer(id,changes){
        const copies=playerCopies(id),previousNames=new Set(copies.map(player=>player.name).filter(name=>name&&name!==changes.name));
        copies.forEach(p=>Object.assign(p,changes));
        // Queue and court entries may be independently restored from saved session data.
        // Update every matching player record in the active session, not just the first copy in each list.
        const visited=new WeakSet(),apply=value=>{if(!value||typeof value!=='object'||visited.has(value))return;visited.add(value);if(value.id===id&&typeof value.name==='string')Object.assign(value,changes);Object.values(value).forEach(apply);};
        apply(state.current);
        if(changes.name&&previousNames.size)state.current?.completedMatches?.forEach(match=>['teamA','teamB'].forEach(side=>{if(Array.isArray(match[side]))match[side]=match[side].map(name=>previousNames.has(name)?changes.name:name);}));
    }
    ...
    function renderActive(){
        const s=state.current;if(!s){show('home');return;}if(s.mode==='team'){s.gamesPerPlayer=Math.min(20,Math.max(1,Number(s.gamesPerPlayer)||initial.setup.gamesPerPlayer));if(!Array.isArray(s.teamPairPlan))s.teamPairPlan=createTeamPairPlan(s.players,s.teamCount,s.gamesPerPlayer);s.players.forEach(player=>{const completed=(player.gamesPlayed||0)>=s.gamesPerPlayer;if(completed&&playerStatus(player)!=='completed')updatePlayer(player.id,{status:'completed'});if(!completed&&playerStatus(player)==='completed')updatePlayer(player.id,{status:'ready'});});}else{s.players.filter(player=>playerStatus(player)==='completed').forEach(player=>updatePlayer(player.id,{status:'ready'}));}
        s.matches=s.matches||emptyCourts(s.courts);s.waiting=s.waiting||[];s.upNext=s.upNext||[];
        const seenWaitingIds=new Set();s.waiting=s.waiting.filter(player=>player?.id&&!seenWaitingIds.has(player.id)&&(seenWaitingIds.add(player.id),true));
        const completedPlayerIds=new Set(s.mode==='team'?s.players.filter(player=>(player.gamesPlayed||0)>=s.gamesPerPlayer).map(player=>player.id):[]);let removedCompletedFromQueue=false;
        s.upNext.forEach(next=>{const before=next.teamA.length+next.teamB.length;next.teamA=next.teamA.filter(player=>!completedPlayerIds.has(player.id));next.teamB=next.teamB.filter(player=>!completedPlayerIds.has(player.id));removedCompletedFromQueue||=before!==next.teamA.length+next.teamB.length;});
        // A ready player belongs in exactly one place: waiting, a court, or an Up Next queue.
        // Restore players orphaned by older automatic-queue behavior back to the waiting pool.
        const assignedPlayerIds=new Set([...(s.upNext||[]).flatMap(match=>[...(match.teamA||[]),...(match.teamB||[])]),...(s.matches||[]).flatMap(match=>[...(match.teamA||[]),...(match.teamB||[]),...(match.manualSlots||[]),...(match.nextQueue||[])])].filter(Boolean).map(player=>player.id)),waitingIds=new Set(s.waiting.map(player=>player.id));
        // Older saved sessions can leave the master record marked queued/away after its
        // waiting-list copy was restored as ready. A player who is ready in the waiting
        // pool and is not assigned anywhere must be selectable without toggling status.
        s.waiting.forEach(waitingPlayer=>{
            const master=s.players.find(player=>player.id===waitingPlayer.id);
            if(master&&isAvailable(waitingPlayer)&&!isAvailable(master)&&playerStatus(master)!=='completed'&&!assignedPlayerIds.has(waitingPlayer.id))updatePlayer(waitingPlayer.id,{status:'ready'});
        });
        const restoredPlayers=s.players.filter(player=>isAvailable(player)&&!assignedPlayerIds.has(player.id)&&!waitingIds.has(player.id));
        if(restoredPlayers.length){const queuedAt=new Date().toISOString();s.waiting.push(...restoredPlayers.map(player=>({...player,queuedAt:player.queuedAt||queuedAt})));}if(restoredPlayers.length||removedCompletedFromQueue)save();
        ...
        $$('[data-start]').forEach(b=>b.onclick=()=>{const m=s.matches.find(x=>x.id===b.dataset.start),capped=playersAtGameLimit([...(m?.teamA||[]),...(m?.teamB||[])],s);if(!m||capped.length){if(capped.length)toast(`${capped.map(player=>player.name).join(', ')} ${capped.length===1?'has':'have'} reached the game limit`);return;}m.started=true;m.startedAt=new Date().toISOString();m.completedAt=null;m.scoreA=0;m.scoreB=0;save();renderActive();});
        ...
        const waitingPlayerIds=new Set(s.waiting.map(player=>player.id));
        const teamList=s.mode==='team'?`<div class="active-game-limit"><div><label class="section-label" for="active-games-per-player">Games per player</label><p class="hint">Updates every player’s required and remaining games.</p><div class="match-plan"><span><strong>Players:</strong> ${s.players.length}</span><span><strong>Games per player:</strong> ${s.gamesPerPlayer}</span><span><strong>Total matches required:</strong> ${totalMatchesLabel}</span></div></div><input class="field" id="active-games-per-player" type="number" min="1" max="20" step="1" value="${s.gamesPerPlayer}" aria-label="Games per player"></div><div class="team-rosters">${Array.from({length:s.teamCount||0},(_,index)=>{const members=s.players.filter(player=>player.teamIndex===index),name=s.teamNames?.[index]||teamPalette[index]?.[0]||`Team ${index+1}`;return `<section class="team-roster" style="--team-color:${teamColor(index)}"><div class="team-roster-head"><input class="team-roster-name" data-active-team-name="${index}" value="${escapeHtml(name)}" maxlength="40" aria-label="Team ${index+1} name"><span class="team-roster-count">${members.length}/${s.teamSize||0} players</span></div>${members.length?`<div class="team-stats-head"><span>Player</span><span>Played</span><span>Required</span><span>Remaining</span><span>W-L</span><span>Status</span></div><div class="team-player-grid">${members.map(player=>{const played=player.gamesPlayed||0,remaining=Math.max(0,s.gamesPerPlayer-played),assignment=playerAssignment(player,s),inWaitingPool=waitingPlayerIds.has(player.id),status=remaining===0?'Completed':assignment||(inWaitingPool?playerStatusLabel(player):'Reserved'),statusClass=remaining===0?'completed':(assignment||(inWaitingPool?playerStatus(player):'queued')).toLowerCase();return `<button class="waiting-player" data-waiting-player="${player.id}" type="button"><span class="waiting-avatar">${escapeHtml(playerInitials(player))}</span><span class="waiting-name">${escapeHtml(player.name)}</span><span class="team-player-stats"><span>${played}</span><span>${s.gamesPerPlayer}</span><span>${remaining}</span><span>${player.wins||0}-${player.losses||0}</span><span class="availability-pill ${statusClass}">${status}</span></span></button>`;}).join('')}</div>`:'<p class="hint" style="padding:.7rem .85rem">No players assigned.</p>'}</section>`;}).join('')}</div>`:null;
        ...
        if(teamList){$$('.team-roster-head',$('#waiting')).forEach((head,index)=>head.insertAdjacentHTML('afterbegin',teamLogoUpload(index,'active')));const remainingAppearances=s.players.reduce((sum,player)=>sum+Math.max(0,s.gamesPerPlayer-(player.gamesPlayed||0)),0),remainingMatches=remainingAppearances/4;$('.match-plan',$('#waiting'))?.insertAdjacentHTML('beforeend',`<span><strong>Remaining matches:</strong> ${Number.isInteger(remainingMatches)?remainingMatches:remainingMatches.toFixed(2)}</span>`);$('#waiting').insertAdjacentHTML('beforeend',standingsSummary+matchupSummary);$$('.team-player-grid .waiting-name').forEach(nameEl=>{const player=s.players.find(item=>item.name===nameEl.textContent.trim());if(!player)return;const lastQueued=playerCopies(player.id).map(copy=>new Date(copy.queuedAt||s.startedAt).getTime()).filter(Number.isFinite).reduce((latest,time)=>Math.max(latest,time),0),minutes=Math.max(0,Math.floor((Date.now()-lastQueued)/60000));nameEl.insertAdjacentHTML('beforeend',`<small>Waiting ${minutes} min</small>`);if((player.gamesPlayed||0)>=s.gamesPerPlayer)nameEl.closest('.waiting-player')?.classList.add('completed-player');});}
        const activeGameLimit=$('#active-games-per-player');if(activeGameLimit)activeGameLimit.onchange=()=>{const value=Number(activeGameLimit.value),maximumPlayed=Math.max(0,...s.players.map(player=>player.gamesPlayed||0));if(!Number.isInteger(value)||value<1||value>20){activeGameLimit.value=s.gamesPerPlayer;return;}if(value<maximumPlayed){toast(`Cannot set the limit below ${maximumPlayed}: a player has already played ${maximumPlayed} games`);activeGameLimit.value=s.gamesPerPlayer;return;}s.gamesPerPlayer=value;state.setup.gamesPerPlayer=value;save();renderActive();};
        $$('[data-active-team-name]').forEach(input=>input.oninput=()=>{s.teamNames=s.teamNames||[];s.teamNames[Number(input.dataset.activeTeamName)]=input.value;save();});
        ...
        $('#match-history').innerHTML=history.length?`<section class="match-history"><div class="match-history-toolbar"><h3 class="match-history-title">Match history · ${history.length}</h3><input class="field match-history-search" id="match-history-search" type="search" placeholder="Search player, team, round, score" aria-label="Search match history"></div><div class="match-history-card">${history.map((item,index)=>{const aWinner=item.winner==='a',bWinner=item.winner==='b',noWinner=!item.winner,noScore=Number(item.scoreA)===0&&Number(item.scoreB)===0,matchIndex=s.completedMatches.length-1-index;const scores=noWinner?'<span class="no-winner-label">No winner</span>':noScore?`<span class="${aWinner?'winner-mark':'loser-mark'}">${aWinner?'W':'—'}</span><span class="${bWinner?'winner-mark':'loser-mark'}">${bWinner?'W':'—'}</span>`:`<span class="${aWinner?'winning-score':''}">${item.scoreA}</span><span class="${bWinner?'winning-score':''}">${item.scoreB}</span>`;return `<div class="history-match ${noWinner?'no-winner-row':''}"><span class="history-round">R${item.round||1}</span><div class="history-teams"><span class="${aWinner?'winner':''}">${escapeHtml(item.teamA.join(' & '))}</span><span class="${bWinner?'winner':''}">${escapeHtml(item.teamB.join(' & '))}</span></div><div class="history-score">${scores}<span class="history-duration" title="Match duration">${matchDuration(item.startedAt,item.completedAt)}</span><button class="history-edit" data-edit-history="${matchIndex}" type="button">Edit result</button></div></div>`}).join('')}<p class="match-history-empty" id="match-history-empty" hidden>No matches found.</p></div></section>`:'';
        if(s.mode==='team')$$('.history-match').forEach((row,index)=>{const item=history[index];if(!item)return;const teams=$$('.history-teams span',row),indices=[item.teamAIndex,item.teamBIndex],colors=indices.map(teamIndex=>teamPalette[teamIndex]?.[1]||'#1673d1');row.style.background=`linear-gradient(90deg,${colors[0]}24,${colors[1]}24)`;row.style.borderLeftColor=colors[0];indices.forEach((teamIndex,side)=>{if(teamIndex===undefined)return;const name=s.teamNames?.[teamIndex]||teamPalette[teamIndex]?.[0]||`Team ${teamIndex+1}`;teams[side].insertAdjacentHTML('afterbegin',`<small style="color:${colors[side]}">${escapeHtml(name)} · </small>`);});});
        const historySearch=$('#match-history-search'),historyEmpty=$('#match-history-empty');if(historySearch)historySearch.oninput=()=>{const query=historySearch.value.trim().toLocaleLowerCase(),rows=$$('.history-match',$('#match-history')),visible=rows.filter(row=>row.textContent.toLocaleLowerCase().includes(query));rows.forEach(row=>row.hidden=!row.textContent.toLocaleLowerCase().includes(query));if(historyEmpty)historyEmpty.hidden=visible.length>0;};
        $$('[data-edit-history]').forEach(button=>button.onclick=()=>editRecordedMatchWinner(Number(button.dataset.editHistory)));
    ...
    function showTeamStandings(){const s=state.current;if(!s)return;const teams=Array.from({length:s.teamCount||0},(_,index)=>{const members=s.players.filter(player=>player.teamIndex===index),wins=Math.round(members.reduce((sum,player)=>sum+(player.wins||0),0)/2),losses=Math.round(members.reduce((sum,player)=>sum+(player.losses||0),0)/2);return {index,name:s.teamNames?.[index]||teamPalette[index]?.[0]||`Team ${index+1}`,games:wins+losses,wins,losses,points:wins};}).sort((a,b)=>b.points-a.points||b.wins-a.wins||a.losses-b.losses||a.name.localeCompare(b.name));standingsPanel.innerHTML=`<div class="standings-head"><div><h3 id="standings-title">Team Standings</h3><p>${escapeHtml(s.name)} · ${s.played||0} matches</p></div><button class="standings-close" id="standings-close" aria-label="Close">×</button></div><div class="summary-table"><div class="summary-table-head"><span style="grid-column:1/3">Rank</span><span>Team</span><span>Games</span><span>Wins</span><span>Losses</span><span>Points</span></div>${teams.map((team,index)=>`<div class="summary-player"><div class="team-rank-cell"><span class="standing-rank leader">${index+1}</span></div><span class="team-rank-details"><strong>${escapeHtml(team.name)}</strong><small class="team-win-rate"></small></span><span class="team-cell">${teamLogoMarkup(team.index,s)}</span><span>${team.games}</span><span>${team.wins}</span><span>${team.losses}</span><span>${team.points}</span></div>`).join('')}</div><div class="standings-actions"><button class="standings-back" id="standings-back" aria-label="Back to game">Back to Game</button></div>`;standingsDialog.hidden=false;$('#standings-close').onclick=hideStandings;$('#standings-back').onclick=hideStandings;}
    function decorateTeamStandings(){standingsPanel.classList.add('team-standings-large');const trophies=['🥇','🥈','🥉'];$$('.summary-player',standingsPanel).forEach((row,index)=>{const cells=[...row.children],games=Number(cells[3]?.textContent)||0,wins=Number(cells[4]?.textContent)||0,rate=games?Math.round(wins/games*100):0,rank=cells[0]?.querySelector('.standing-rank'),winRate=cells[1]?.querySelector('.team-win-rate');if(rank)rank.textContent=trophies[index]||`#${index+1}`;if(winRate)winRate.textContent=`${rate}% win rate`;});}
    ...
    function confirmEndSession(){if(!state.current)return;const teamSession=state.current.mode==='team',incomplete=teamSession?state.current.players.filter(player=>(player.gamesPlayed||0)<state.current.gamesPerPlayer):[];if(incomplete.length){toast(`${incomplete.length} player${incomplete.length===1?' needs':'s need'} more games before the session can end`);return;}endDialog.classList.remove('selector');endDialogPanel.innerHTML=`<div class="end-dialog-copy"><h3 id="end-dialog-title">End session?</h3><p>${teamSession?`Every player has completed ${state.current.gamesPerPlayer} games.`:'No more matches can be generated after ending.'} You’ll see the complete session summary next.</p></div><div class="confirm-actions"><button id="cancel-end-session">Cancel</button><button id="confirm-end-session" style="color:#ff6b66">End session</button></div>`;endDialog.hidden=false;$('#cancel-end-session').onclick=hideEndDialog;$('#confirm-end-session').onclick=finishSession;}
    let endingSession=false;
    async function finishSession(){
        if(!state.current||endingSession)return;
        const session=structuredClone(state.current);
        session.id=session.id||newId();
        session.endedAt=new Date().toISOString();
        endingSession=true;
        const confirmButton=$('#confirm-end-session');
        if(confirmButton){confirmButton.disabled=true;confirmButton.textContent='Saving…';}
        try{
            if(state.keepHistory)await archiveSession(session);
            state.summary=session;
            if(state.keepHistory)state.history.unshift(session);
            state.current=null;
            state.players=[];
            state.setup={...initial.setup};
            $('#session-name').value='';
            renderSetup();
            renderPlayers();
            hideEndDialog();
            ...
            // Keep both the active session and an on-device copy so it can be retried safely.
            persistOffline(persistedSnapshot());
            toast('Session was not saved. It is still open—check the connection and tap End Session again.');
            if(confirmButton){confirmButton.disabled=false;confirmButton.textContent='End session';}
        }finally{endingSession=false;}
    }
    $('#end-session').onclick=confirmEndSession;
    $('#summary-done').onclick=()=>show('home');
    function closeOpenMatch(record,winner=null){const s=state.current,m=currentOpenMatch();if(!s||!m)return;const completedAt=new Date().toISOString(),matchPlayers=[...m.teamA,...m.teamB];if(record){s.played=(s.played||0)+1;const winningTeam=winner==='a'?m.teamA:winner==='b'?m.teamB:[],losingTeam=winner==='a'?m.teamB:winner==='b'?m.teamA:[],bump=(player,field)=>{const canonical=s.players.find(p=>p.id===player.id);[...new Set([player,canonical].filter(Boolean))].forEach(p=>p[field]=(p[field]||0)+1);};matchPlayers.forEach(p=>bump(p,'gamesPlayed'));winningTeam.forEach(p=>bump(p,'wins'));losingTeam.forEach(p=>bump(p,'losses'));s.completedMatches=s.completedMatches||[];s.completedMatches.push({court:m.court,round:s.round,teamA:m.teamA.map(p=>p.name),teamB:m.teamB.map(p=>p.name),teamAIndex:m.teamAIndex,teamBIndex:m.teamBIndex,scoreA:m.scoreA||0,scoreB:m.scoreB||0,winner,startedAt:m.startedAt,completedAt});}
        // A completed game starts a fresh wait. Keep the roster and court records in sync
        // so both the display and automatic queue use the same timestamp.
        if(record)matchPlayers.forEach(player=>{const canonical=s.players.find(item=>item.id===player.id);[...new Set([player,canonical].filter(Boolean))].forEach(copy=>copy.queuedAt=completedAt);});
        s.waiting.push(...matchPlayers);m.teamA=[];m.teamB=[];m.started=false;m.startedAt=null;m.completedAt=null;m.scoreA=0;m.scoreB=0;m.done=false;openMatchId=null;hideEndDialog();save();show('active');}
    function editRecordedMatchWinner(matchIndex){
        const s=state.current,item=s?.completedMatches?.[matchIndex];
        if(!item)return;
        const setWinner=winner=>{
            if(item.winner===winner){hideEndDialog();return;}
            const adjust=(names,field,amount)=>names.forEach(name=>{const player=s.players.find(candidate=>candidate.name===name);if(player)player[field]=Math.max(0,(player[field]||0)+amount);});
            if(item.winner==='a'){adjust(item.teamA,'wins',-1);adjust(item.teamB,'losses',-1);}
            if(item.winner==='b'){adjust(item.teamB,'wins',-1);adjust(item.teamA,'losses',-1);}
            if(winner==='a'){adjust(item.teamA,'wins',1);adjust(item.teamB,'losses',1);}
            if(winner==='b'){adjust(item.teamB,'wins',1);adjust(item.teamA,'losses',1);}
            item.winner=winner;
            hideEndDialog();
            save();
            renderActive();
        ...
        endDialog.classList.add('selector');
        endDialogPanel.innerHTML=`<div class="end-dialog-copy"><h3 id="end-dialog-title">Edit match result</h3><p>Choose the correct winner for this completed match.</p></div><div class="winner-options"><button data-correct-winner="a">${escapeHtml(item.teamA.join(' & '))} won</button><button data-correct-winner="b">${escapeHtml(item.teamB.join(' & '))} won</button><button class="no-winner" data-correct-winner="">No winner</button></div><div class="winner-cancel-wrap"><button class="winner-cancel" id="winner-cancel">Cancel</button></div>`;
        endDialog.hidden=false;
        $$('[data-correct-winner]',endDialogPanel).forEach(button=>button.onclick=()=>setWinner(button.dataset.correctWinner||null));
        $('#winner-cancel').onclick=hideEndDialog;
    }
