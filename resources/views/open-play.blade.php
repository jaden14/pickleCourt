...
<head>
    ...
    <style>
        ...
        * { box-sizing:border-box; }
        ...
        .team-player-grid .waiting-avatar { grid-column:1; grid-row:1; }
        ...
        .score-scene-player.team-a-player:nth-child(2) { --player-color:#16a66a; top:82px; left:34%; }
        .score-scene-player.team-b-player:nth-child(3) { --player-color:#ef7b17; top:27px; right:34%; }
        .score-scene-player.team-b-player:nth-child(4) { --player-color:#8b55ca; top:82px; right:15%; }
...
(() => {
    ...
    const scoreCourtPlayers = (teamA,teamB) => `${teamA.map(player=>`<span class="score-scene-player team-a-player" title="${escapeHtml(player.name)}">${escapeHtml(playerInitials(player))}</span>`).join('')}${teamB.map(player=>`<span class="score-scene-player team-b-player" title="${escapeHtml(player.name)}">${escapeHtml(playerInitials(player))}</span>`).join('')}<span class="score-court-ball"></span>`;
    ...
    function renderActive(){
        ...
        if(s.mode==='team')$$('.up-next-card').forEach((card,index)=>{const item=upNext[index],labels=$$('label',card),panels=$$('.up-next-team',card),indices=[item.teamAIndex,item.teamBIndex];indices.forEach((teamIndex,side)=>{const color=teamPalette[teamIndex]?.[1]||'#1673d1',name=teamIndex===undefined?`Team ${side===0?'A':'B'}`:(s.teamNames?.[teamIndex]||teamPalette[teamIndex]?.[0]||`Team ${teamIndex+1}`);labels[side].textContent=name;labels[side].style.color=color;panels[side].style.backgroundColor=`${color}20`;panels[side].style.borderTop=`3px solid ${color}`;});card.style.borderColor=teamPalette[item.teamAIndex]?.[1]||'#a7edc3';});
